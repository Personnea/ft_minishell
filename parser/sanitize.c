...
int	sanitize_pipe(t_parser *token, t_parser **head)
{
	...
	if (get_prev_token_non_space(token)->type != CMD
		...
		&& get_prev_token_non_space(token)->type != ENVVAR
		&& get_prev_token_non_space(token)->type != EXIT_STATUS
		&& get_prev_token_non_space(token)->type != FILENAME
		&& get_prev_token_non_space(token)->type != IS_DELIMITER
		&& get_prev_token_non_space(token)->type != DQUOTE
		&& get_prev_token_non_space(token)->type != SQUOTE)
		return (error_near_pipe(), 1);
	return (0);
}
